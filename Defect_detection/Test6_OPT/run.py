...
from transformers import (WEIGHTS_NAME, AdamW, get_linear_schedule_with_warmup,
                          ...
                          OPTConfig, OPTModel, OPTForSequenceClassification, AutoTokenizer)
...
MODEL_CLASSES = {
    ...
    'opt': (OPTConfig, OPTForSequenceClassification, AutoTokenizer)
}
    ...
    def __init__(self, tokenizer, args, file_path=None):
        ...
        if 'train' in file_path:
            for idx, example in enumerate(self.examples[:1]):
                    ...
                    logger.info("input_tokens: {}".format([x.replace('\u0120', '_') if x is not None else '' for x in example.input_tokens]))
...
def train(args, train_dataset, model, tokenizer):
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        ...
        preds = train_logits[:, 1] > 0.5

        train_acc = np.mean(train_labels == preds)
...
def test(args, model, tokenizer):
    ...
    labels=np.concatenate(labels,0)
    preds=logits[:,1]>0.5
    with open("../evaluator/predictions.txt", 'w') as f:
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default='./code', type=str,
                        help="The model checkpoint for weights initialization.")
    parser.add_argument("--tokenizer_name", default="./code", type=str,
